...
double Sensors::waveError(int measurementIndex, int iterator, int xShift, int amplitude, int period) {
    return pow((samples[measurementIndex][iterator] - simulateWave(input[measurementIndex].yShift, xShift, amplitude, iterator, period)), 2);
}
...
void Sensors::recordSamples() {
    ...
    for(int a = 0; a < 2; a++) { // The first iteration is to calibrate measurementDuration
        ...
        for(unsigned int i = 0; i < measurement_samples; i++) {
            for(unsigned int j = 0; j < input_count; j++) {
                samples[j][i] = simulateWave(input[j].yShift, input[j].xShift, input[j].amplitude, i, p);
            }
        }
        if(a < 1) {
          sampleTime += micros();
    ...
    switch(mode) {
        case -1:
            for(unsigned int j = 0; j < input_count; j++) {
                ...
                for(unsigned int i = 0; i < measurement_samples; i++) {
                    Serial.println(String::format("%d, %f", i*measurementDuration, simulateWave(input[j].yShift, input[j].xShift, input[j].amplitude, i, input[j].period)));
                }
            }
            break;
        default:
